...
#ifndef SRCCPP_DISPLAYCPP_GLCD_H_
#define SRCCPP_DISPLAYCPP_GLCD_H_
#include "common.h"
...
public:
	...
	virtual ~GLCD();
	void m_ctrloff(void);
	void m_displayon(void);
	void m_displayoff(void);
	void m_setcolumn(uint8_t Columvalue);
	void m_setpage(uint8_t x);
	void m_setstartline(uint8_t z);
	void m_lcddata(uint8_t *value,uint16_t limit);
	void m_lcdputs1(uint8_t y,uint8_t x,char *str);
	void m_lcdputs2(uint8_t y,uint8_t x,uint8_t *str);
	void m_picture(uint8_t * val);
	void m_delay(unsigned int x);
	void m_delay(unsigned int x,uint8_t);
	void m_IOWritethedata(const uint8_t finput);
	void m_clrlcd(void);
	unsigned char check[15]="  Powermeter ";
private:
	unsigned int i{0},k{0};
	uint8_t c;
	uint8_t z{0};

	uint8_t Character8x8[2056] = {
		  0,   0,   0,   0,   0,   0,   0,   0,
...
#endif /* SRCCPP_DISPLAYCPP_GLCD_H_ */
